...
def _run_daily_job():
    """Daily scheduled job: find birthday matches and send emails."""
...
def _notify_admin(config: dict, result: dict):
    """Send a summary email to the sender address."""
    import smtplib
...
def update_schedule(send_time: str = None, auto_send: bool = None, timezone: str = None):
    """Update the scheduler settings and reschedule if needed."""
    global _scheduler
    config = load_config()
...
def get_next_run() -> str:
    """Return the next scheduled run time as a human-readable string."""
    if _scheduler:
