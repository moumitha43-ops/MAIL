...
# ──────────────────────────────────────────────────────
#  CARD RENDERING  (Playwright → PNG screenshot)
# ──────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────
#  GMAIL API AUTH  (reads GOOGLE_TOKEN_JSON env var)
# ──────────────────────────────────────────────────────

def _get_gmail_service():
    token_json = os.getenv("GOOGLE_TOKEN_JSON")
    if not token_json:
        raise RuntimeError("GOOGLE_TOKEN_JSON environment variable not set")
...
def _send_one(sender: str, match: dict, template_html: str, max_retries: int = 3) -> dict:
    name       = match["name"]
    email      = match["email"]
    rollnumber = match["rollnumber"]

    # Duplicate guard (uncomment to activate):
